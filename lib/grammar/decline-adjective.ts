...
/**
 * Déclinaison des adjectifs (forme longue), RULE-BASED comme decline.ts.
 *
 * Trois familles :
...
export function declineAdjective(
  ...
  if (targetCase === "nominative") {
    ...
    return { case: targetCase, form: r.form, ruleApplied: r.rule, isIrregular: false };
  }
  ...
  if (targetCase === "accusative") {
    ...
    if (!plural && gender === "neuter") {
      const r = nonAccusativeForm(adj, "nom", gender, plural, stem, lastLetter);
      return { case: targetCase, form: r.form, ruleApplied: "adjectif neutre : accusatif = nominatif", isIrregular: false };
    }
    ...
    if (animacy === "animate") {
      ...
      return { case: targetCase, form: r.form, ruleApplied: "adjectif animé : accusatif = génitif", isIrregular: false };
    }
    const r = nonAccusativeForm(adj, "nom", gender, plural, stem, lastLetter);
    return { case: targetCase, form: r.form, ruleApplied: "adjectif inanimé : accusatif = nominatif", isIrregular: false };
  }
