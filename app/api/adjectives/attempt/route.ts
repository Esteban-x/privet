...
import { checkAdjectiveAnswer, getSkill } from "@/lib/adjectives/exercises";
import { allowPractice } from "@/lib/practice/quota";

/**
 * Seule autorité sur la justesse d'une réponse du module « Accord de
 * l'adjectif », comme app/api/aspect/attempt l'est pour l'aspect.
 *
 ...
 * (lib/adjectives/exercises.ts) : impossible de gonfler sa progression, et
...
export async function POST(req: Request) {
  ...
  const correct = checkAdjectiveAnswer(itemId, answer);
  if (correct === null) {
  ...
  const { error } = await supabase.from("adjective_progress").upsert(
    {
      user_id: user.id,
      skill_id: skill,
      attempts,
