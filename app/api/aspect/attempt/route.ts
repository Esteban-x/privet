...
import { checkAspectAnswer, getSkill } from "@/lib/aspect/exercises";
 ...
 * (lib/aspect/exercises.ts) : impossible de gonfler sa progression, et
...
export async function POST(req: Request) {
  ...
  const correct = checkAspectAnswer(itemId, answer);
  if (correct === null) {
  ...
  const { error } = await supabase.from("aspect_progress").upsert(
    {
      user_id: user.id,
      skill_id: skill,
      attempts,
