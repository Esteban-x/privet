...
--
-- L'état du schéma tel qu'il tournait avant l'adoption des migrations.
-- Elle reprend l'ancien `supabase/schema.sql`, qui était rejoué à la main
-- dans l'éditeur SQL : tout y est donc idempotent (`if not exists`,
-- `drop policy if exists`), et la rejouer sur une base déjà en service ne
-- casse rien. C'est ce qui permet de l'appliquer sans risque à une base
-- existante plutôt que de devoir la déclarer appliquée à la main.
--
-- Les migrations suivantes, elles, n'ont plus à être idempotentes : elles
-- ne tournent qu'une fois, dans l'ordre, et Supabase tient le compte.
-- ════════════════════════════════════════════════════════════════
...
create table if not exists public.activity_log (
  id         uuid primary key default gen_random_uuid(),
  user_id    uuid not null references auth.users(id) on delete cascade,
  kind       text not null,             -- 'case' | 'motion' | 'aspect' | 'participle' | 'adjective' | 'vocab' | 'reading'
  correct    boolean,
  meta       jsonb,
  created_at timestamptz default now()
);
...
declare t text;
begin
  foreach t in array array['level_tests','case_progress','case_trigger_progress','motion_progress','aspect_progress','participle_progress','adjective_progress','srs_cards','activity_log','vocab_lists','vocab_words','reading_texts']
  loop
