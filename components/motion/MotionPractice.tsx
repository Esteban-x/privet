...
export default function MotionPractice({
  ...
  const [exercise, setExercise] = useState<MotionExercise | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.resolve(generateMotionExercise(skill)).then((next) => {
          ...
          <div key={round} className="animate-fade-in">
            ...
            {exercise.schema && (
              <figure className="mt-4 flex flex-col items-center rounded-[14px] border border-border bg-bg px-4 py-5">
                <TrajectoryDiagram schema={exercise.schema} mode={exercise.mode} />
                <figcaption className="mt-2 font-display text-xs text-muted">
                  {SCHEMA_LABEL[exercise.schema]}
                </figcaption>
              </figure>
            )}
